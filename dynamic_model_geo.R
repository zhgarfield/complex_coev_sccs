...
diff_RI_TSD <- delta_theta_RI - delta_theta_TSD

delta_theta_df <- data.frame(
  delta_theta = c(delta_theta_RI, delta_theta_TSD, diff_RI_TSD),
  resp = rep(c("RI", "TSD", "TSD - RI"), each = n_samps)
)

round(sum(delta_theta_RI > 0)/n_samps, 2)
round(sum(delta_theta_TSD > 0)/n_samps, 2)
round(sum(diff_RI_TSD > 0)/n_samps, 2)
...
p_delta_theta <- ggplot(delta_theta_df, aes(x=delta_theta)) + 
  ...
  annotate("text",label=expression("RI" %->% "TSD"), x=1.2, y=1.5, color="deepskyblue4", size=3.5) +
  annotate("text",label=expression(paste("    PP"[">0"]," > 0.99")), x=1.2, y=1.3, color="deepskyblue4", size=3.5) +
  annotate("text",label=expression("TSD" %->% "RI"), x=0.45, y=2.25, color="gray10", size=3.5) +
  annotate("text",label=expression(paste("    PP"[">0"]," = 0.77")), x=0.45, y=2.05, color="gray10", size=3.5) +
  annotate("text",label=expression(paste(Delta, "(TSD" %->% "RI - RI" %->% "TSD)")), x=-1.3, y=1.4, color="darkorange", size=3.5) +
  annotate("text",label=expression(paste("    PP"[">0"]," = 0.03")), x=-1.3, y=1.2, color="darkorange", size=3.5) +
  theme_minimal() + 
  scale_x_continuous(limits=c(-2,2)) + 
  scale_y_continuous(expand=c(0,0), limits=c(0,2.6)) +
  scale_color_manual(values=c("gray10", "#60C4EB", "darkorange")) +
  scale_fill_manual(values=c("gray10", "#60C4EB", NA)) + 
