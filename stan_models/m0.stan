...
for (i in 1:N) {
  hunting[i] ~ ordered_logistic( phy_v[i,J-1] , c_hunt );
  
  if (storage[i] == -99) {
  target += log_mix( inv_logit( a_storage + phy_v[i,J] ),
  bernoulli_logit_lpmf( 1 | a_storage + phy_v[i,J] ),
  bernoulli_logit_lpmf( 0 | a_storage + phy_v[i,J] )
  );
  }
  
  else
  storage[i] ~ bernoulli_logit( a_storage + phy_v[i,J] );
}
}
...
for (i in 1:N) {
  ...
  if (storage[i] == -99) {
    store_hat[i] = bernoulli_logit_rng(a_storage + phy_v[i,J] );
  }
  
  for (j in 1:J-2) {
    log_lik[i,j] = ordered_logistic_lpmf( y[i,j] | phy_v[i,j], c[j,] );
  }
  log_lik[i,J-1] = ordered_logistic_lpmf( hunting[i] | phy_v[i,J-1], c_hunt );
  log_lik[i,J] = bernoulli_logit_lpmf( store_hat[i] | a_storage + phy_v[i,J] );
}
