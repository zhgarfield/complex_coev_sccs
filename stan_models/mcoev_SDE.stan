...
model{
  ...
  hunting[i] ~ ordered_logistic( eta[i,1]*lambda[J-1], c_hunt );
  
  // Mix over missingness in food storage
  if (storage[i] == -99) {
  target += log_mix( inv_logit( a_storage + eta[i,1]*lambda[J] ),
  bernoulli_logit_lpmf( 1 | a_storage + eta[i,1]*lambda[J] ),
  bernoulli_logit_lpmf( 0 | a_storage + eta[i,1]*lambda[J] )
  );
  }
  
  else
  storage[i] ~ bernoulli_logit( a_storage + eta[i,1]*lambda[J] );
}
}
